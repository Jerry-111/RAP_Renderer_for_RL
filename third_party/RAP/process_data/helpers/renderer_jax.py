...
# JAX is used for transform/projection math. NumPy remains where OpenCV requires
# host arrays and in mutable CPU-side drawing utilities.
...
@jax.jit
def _build_se3_jit(R: jnp.ndarray, t: jnp.ndarray) -> jnp.ndarray:
    upper = jnp.concatenate([R, t.reshape(3, 1)], axis=1)
    lower = jnp.array([[0.0, 0.0, 0.0, 1.0]], dtype=jnp.float32)
    return jnp.concatenate([upper, lower], axis=0)


@jax.jit
def _yaw_to_rot_jit(yaw: jnp.ndarray) -> jnp.ndarray:
    c = jnp.cos(yaw)
    s = jnp.sin(yaw)
    return jnp.array(
        [
...
@jax.jit
def _world_to_camera_T_jit(
    lidar_pos: jnp.ndarray,
    lidar_yaw: jnp.ndarray,
    cam2lidar_t: jnp.ndarray,
    cam2lidar_R: jnp.ndarray,
) -> jnp.ndarray:
    T_w_lidar = _build_se3_jit(_yaw_to_rot_jit(lidar_yaw), lidar_pos)
    T_cam_lidar = _build_se3_jit(cam2lidar_R, cam2lidar_t)
    T_w_cam = T_w_lidar @ T_cam_lidar
    return jnp.linalg.inv(T_w_cam)


@jax.jit
def _project_points_cam_jit(
...
def _transform_points_w2c_jit(points_world: jnp.ndarray, T_w2c: jnp.ndarray) -> jnp.ndarray:
    return (T_w2c[:3, :3] @ points_world.T + T_w2c[:3, 3:4]).T


def _project_homogeneous_jit(points_cam: jnp.ndarray, K: jnp.ndarray) -> jnp.ndarray:
...
def project_points_cam(points_cam: np.ndarray,
    ...
    """
    H, W = img_hw
    uv, valid = _project_points_cam_jit(
        _to_jax(points_cam),
        _to_jax(K),
        _to_jax(H),
        _to_jax(W),
    )
    return np.asarray(uv, dtype=np.int32), np.asarray(valid, dtype=bool)
...
def draw_polyline_depth(canvas, polyline3d, T_w2c, K, color,
    ...
    # ---------- 1. 一次性变换 & 投影 ---------- #
    pts_cam = np.asarray(
        _transform_points_w2c_jit(_to_jax(polyline3d), _to_jax(T_w2c)),
        dtype=np.float32,
    )
    z = pts_cam[:, 2]
    proj_uv = np.asarray(
        _project_homogeneous_jit(_to_jax(pts_cam), _to_jax(K)),
        dtype=np.float32,
    ).copy()
...
def draw_polygon_depth(canvas: np.ndarray,
    ...
    # --- World → camera space ------------------------------------------------
    pts_cam = np.asarray(
        _transform_points_w2c_jit(_to_jax(hull3d), _to_jax(T_w2c)),
        dtype=np.float32,
    )
    ...
    uv = np.asarray(
        _project_homogeneous_jit(_to_jax(pts_cam_front), _to_jax(K)),
        dtype=np.float32,
    )
...
def draw_cuboid_at(canvas,
    ...
    pts_cam = np.asarray(
        _transform_points_w2c_jit(_to_jax(world_corners), _to_jax(T_w2c)),
        dtype=np.float32,
    )
...
def draw_heading_arrow(canvas,
    ...
    points_cam = np.asarray(
        _transform_points_w2c_jit(
            _to_jax(np.vstack([p_tail_w, p_head_w])),
            _to_jax(T_w2c),
        ),
        dtype=np.float32,
    )
    pw_tail_c = points_cam[0]
    pw_head_c = points_cam[1]
